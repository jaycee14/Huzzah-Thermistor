...
// the value of the 'other' resistor (to drop total volatge to 1v required by the huzzah )
...
void loop() {
  ...
  String url = "/dataLog/eonTemp.php?";

  url += String("temp=");
  url += String(readThermistor(),2);
  ...
  Serial.end();
  //delay(1000);
  ESP.deepSleep(300 * 1000000); //5 * 60 * 1000,000 = 5 mins
...
float readThermistor (void)
{
  ...
  average = SERIESRESISTOR / average; // Rp
